...
import { enQuiz } from "@/lib/data/english";
...
export function LevelTab() {
  ...
  useEffect(() => {
    const saved = guestStorage.get<number>("en_progress", 62);
    if (typeof saved === "number") setProgress(saved);
  }, []);

  useEffect(() => {
    guestStorage.set("en_progress", progress);
  }, [progress]);
  ...
  return (
    <div className="space-y-5 animate-[fadeInUp_0.4s_ease-out]">
      ...
      >
        ...
        <div className="mb-1 text-base font-bold text-white">مستوى المتوسط (B1)</div>
        <div className="mb-4 text-xs text-white/60">أنت تتقدم بشكل ممتاز! واصل العمل</div>
        <ProgressBar value={progress} gradient={`linear-gradient(90deg,#FFE66D,${EN})`} />
      ...
      <QuizCard
        question={enQuiz.question}
        options={enQuiz.options}
        counter="س 1 / 5"
        accentColor={EN}
        onSpeak={() => speak(enQuiz.speakText, "normal")}
      />
      ...
      {[
        { icon: "🎯", title: "إنجاز رائع!", desc: "أكملت 20 جملة إنجليزية هذا الأسبوع", time: "اليوم" },
        { icon: "📈", title: "تحسن في النطق", desc: "استخدمت ميزة النطق 45 مرة هذا الأسبوع", time: "أمس" },
      ].map((n) => (
        <div
          key={n.title}
